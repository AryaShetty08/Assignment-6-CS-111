...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int i = 0; i<drywet.length; i++)
        {
            for(int j = 0; j < 1; j++)
            {
                if(drywet[i][j] == longitude && drywet[i][j+1] == latitude)
                {
                    row = i;
                    break;
                }
        ...
        for(int j = 2; j<drywet[row].length; j++)
        {
            drywetProbability[counter] = drywet[row][j];
            wetwetProbability[counter] = wetwet[row][j];
            counter++;
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        // COMPLETE THIS METHOD
        int[] forecastPrediction = new int[numberOfDays];
        int counter = 0;
        double rand = 0;
        ...
        if(rand <= 0.5)
        {
            forecastPrediction[counter] = 1;
        }
        else
        {
            forecastPrediction[counter] = 2;
        }
        counter++;
        ...
        while(counter < numberOfDays)
        {
            rand = StdRandom.uniform();
                ...
                if(forecastPrediction[counter-1] == 1 && rand <= wetwetProbability)
                {
                    forecastPrediction[counter] = 1;
                }
                else if(forecastPrediction[counter-1] == 2 && rand <= drywetProbability)
                {
                    forecastPrediction[counter] = 1;
                }
                else 
